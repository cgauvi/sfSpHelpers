...
shp_iso_all_modes <- SfSpHelpers::st_make_isochrone (osm$osm_lines %>% st_centroid() %>% st_transform(crs=4326),
                           buffer_km = 15) 
...
ggplot() + 
  ...
  geom_sf(data=shp_site_buffer %>% st_centroid(), col='salmon') + 
...
ggsave(here('figures', 'isochrones_all_modes_atlanta.png'),bg='white')
...
```{r}
  ...
  osm <- SfSpHelpers::osm_buffer_query_helper(shp_point,buffer_km = 5)
...
## Mapview inspect 
```{r}
 mapview::mapview(shp_point_cap_blanc ,color='salmon') + 
 mapview::mapview(shp_all_roads %>% filter(type=='stairs')) + 
   mapview::mapview(shp_all_roads %>% filter(type!='stairs'))+ 
  mapview::mapview(shp_iso_foot %>% filter(type=='stairs'),color='black') +
  mapview::mapview(shp_iso_foot %>% filter(type!='stairs'),color='black')  
...
get_isochrone_from_city_centroid <- function(city_str, 
                          buffer_size_km = 20,  #at 50 km/h <-> 1.2 min per km, driving for 15 minutes results in 18 km distance 
                          list_modes=c("motorcar"),
                          list_times=c(5,10,15)){
...
shp_isochone <- map_dfr( c('Atlanta', 'Quebec City', 'Montreal'),
...
shp_streets <- map_dfr( c('Atlanta', 'Quebec City', 'Montreal'),
                         ~get_street_network(.x,buffer_size_km = 15))
...
list_plots <- map(c('Atlanta', 'Quebec City', 'Montreal') , plot_city )
...
df_sizes <- map_dfr( c(5,10,15), ~compare_size(shp_isochone, .x))
