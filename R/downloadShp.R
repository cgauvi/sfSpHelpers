...
st_read_remote <- function(url, dirToDownload= here('Data')){
  ...
  destFile <- tryCatch({
    ...
    print(glue("Fatal error with url {url}\n{e}\nsaving to root: {here()}"))
    return( tempfile()  )
  })
...
get_zipped_remote_shapefile <- function(path){
  ...
  if(grepl('http',path)){
    ...
    GeneralHelpers::createDirIfNotExistentRecursive(here("Data",pathRootNoExt))
    ...
    if ( file.exists( shpPath  )){
      return (  st_read(shpPath,quiet=TRUE) )
    }
